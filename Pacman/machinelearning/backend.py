...
alphabet above have been substituted with ASCII symbols.""".strip())
    ...
    def iterate_once(self, batch_size):
        ...
        self.epoch += 1
        # iteration = number of samples in training dataset / batch size 
        # iteration increases with a smaller batch size. 
        # print(self.train_x)
        # print(self.train_x[[5,22,21]])
        for iteration in range(self.train_x.shape[0] // batch_size):
            ...
            example_ids = self.train_buckets[bucket_id, 0] + np.random.choice(
                ...
                size=batch_size)
            # print(f"example={example_ids}")
            # print(self.train_x[example_ids])
    ...
    def get_validation_accuracy(self):
        dev_predicted_probs, dev_predicted, dev_correct = self._predict()
        # print(dev_predicted_probs, dev_predicted, dev_correct)
        dev_accuracy = np.mean(dev_predicted == dev_correct)
        print(dev_accuracy)
        return dev_accuracy
